...
namespace BinaryLand.Input
{
    ...
    public class Stick
    {
        public Stick(Texture2D texture, float deadZoneSize, Rectangle StartRegion, float aliveZoneSize, float aliveZoneFollowFactor, float aliveZoneFollowSpeed, float edgeSpacing)
        {
        ...
        public void SetAsFreefollow() => Style = TouchStickStyle.FreeFollow;

        public void SetAsFree() => Style = TouchStickStyle.Free;

        public Vector2 GetPositionVector(float aliveZoneSize) => StartLocation + (Direction * new Vector2(1, -1) * Magnitude * aliveZoneSize);

        public Vector2 GetRelativeVector(float aliveZoneSize) => Direction * new Vector2(1, -1) * Magnitude * aliveZoneSize;
        ...
        {
            //DrawStringCentered($"L", StartLocation, Color.Black, spriteBatch);
            //DrawStringCentered($"L@L", GetPositionVector(aliveZoneSize), Color.White, spriteBatch);
            ...
            Rectangle dest = new(GetPositionVector(aliveZoneSize).ToPoint(), texture.Bounds.Size);

            spriteBatch.Draw(texture, dest /*GetPositionVector(aliveZoneSize)*/, null, Color.White, 0, origin, SpriteEffects.None, 0);
        }
